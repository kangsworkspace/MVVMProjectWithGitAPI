...
- `Model`은 **비즈니스 로직(데이터 fetch, entity 정의)로직**을 담당하였습니다.  
...
`bindViewModel()`, `bindView()` 함수로 Output에 대한 처리가 모여있기 때문입니다.  
<br>
...
- 원인: 새로운 검색어로 검색할 때 빈 배열로 값을 초기화 하고 `.subscribe`에서 값을 넣도록 작성하였습니다.  
       결과, 구독하는 값이 빈 값으로 초기화 될 때 `EmptyView`가 짧게 보이게 되었습니다.
(기존 코드)
```swift
input.searchTrigger
